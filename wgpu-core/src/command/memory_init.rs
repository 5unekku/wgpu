...
impl BakedCommands {
    ...
    pub(crate) fn initialize_texture_memory(
        ...
        for texture_use in self.texture_memory_actions.drain_init_actions() {
            // Hold `initialization_status` only long enough to collect the ranges
            // needing a clear. `clear_texture` below takes further locks, and we are
            // already holding `Device::trackers` here, so keeping this guard alive
            // across it would widen the set of locks held at once for no reason.
            {
